...
%
%功率平均
p0 = 1/6; p1 = 1/6; p2 = 1/6;
p3 = 1/6; p4 = 1/6; p5 = 1/6;
...
gain2 = sqrt(p2)*abs(randn(1,NN) + 1i*randn(1,NN));
gain3 = sqrt(p3)*abs(randn(1,NN) + 1i*randn(1,NN));
gain4 = sqrt(p4)*abs(randn(1,NN) + 1i*randn(1,NN));
gain5 = sqrt(p5)*abs(randn(1,NN) + 1i*randn(1,NN));
for k = 1:NN
   for kk=1:fs
      index=(k-1)*fs+kk;
...
axis([0 14 10^(-10) 1])
legend('Theoretical BER','BER of uniform power over the 6 rays')
